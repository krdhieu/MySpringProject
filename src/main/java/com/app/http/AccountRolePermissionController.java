...
public class AccountRolePermissionController {
    ...
    public @ResponseBody ResponseEntity<List<AccountRolePermission>> findRolePermissionByAccountId(
            @PathVariable("accountId") Long accountId,
            Authentication authentication
    ) {
        MyUserDetails myUserDetails = (MyUserDetails) userDetailsService.loadUserByUsername(authentication.getName());
        if (myUserDetails.isAdmin() || myUserDetails.isCurrentAccount(accountId))
    ...
    public @ResponseBody ResponseEntity<List<Object[]>> findRolePermissionNameByAccountId(
            @PathVariable("accountId") Long accountId,
            Authentication authentication
    ) {
        MyUserDetails myUserDetails = (MyUserDetails) userDetailsService.loadUserByUsername(authentication.getName());
    ...
    public @ResponseBody ResponseEntity<AccountRolePermission> createAccountRolePermission(@RequestBody AccountRolePermission accountRolePermission) {
        return new ResponseEntity<>(accountRolePermissionService.createAccountRolePermission(accountRolePermission), HttpStatus.CREATED);
    }
    ...
    public @ResponseBody ResponseEntity<Integer> deleteAccountRolePermissionById(@PathVariable("id") Long id) {
        return new ResponseEntity<>(accountRolePermissionService.deleteAccountRolePermissionById(id), HttpStatus.OK);
    }
}
